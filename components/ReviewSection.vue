          ...
          <div class="cr-widget-focalreviews">
            <div class="card-padding">
              <div class="review-header">
                <h3>
                  <span class="a-size-base">Showing 1-8 of {{reviews.length}}</span>
                </h3>
              </div>
              ...
              <!-- Reviews -->
              <div class="review-body" v-for="(review, index) in reviews" :key="review._id">
                ...
                <div class="review-comments">
                  <div class="a-spacing-small">
                    <span class="a-size-base a-color-tertiary">{{review.helpfuls.length }} people found this helpful</span>
                  </div>
                  <div class="cr-helpful-button">
                    <span class="a-button-base">
                      <span class="a-button-inner">
                        <span class="a-button-text" @click="onHelpful(index, review._id)">
...
<script>
    export default {
        ...
        methods: {
          async onHelpful(index, id) {
            try {

              let response = await this.$axios.$post(`/api/helpful/${id}`);
              if (response.success) {
                this.$props.reviews[index].helpfuls.push(response.savedHelpful);
              }
            } 
            catch (error) {
              console.log(error)
